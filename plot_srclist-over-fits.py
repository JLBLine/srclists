...
parser.add_option('-s',"--srclist", help="srclist used to calibrate or peel")
...
parser.add_option('-l','--limits',default=False,help ="Limits for the plot, enter as min,max")

parser.add_option('-f','--fits_file', help = "Fits file to base image on")
...
ax = aplpy.FITSFigure(options.fits_file,figure=fig, subplot=(1,1,1))

if options.limits:
	vmin,vmax = map(float,options.limits.split(','))
	ax.show_colorscale(cmap='gray',vmin=vmin,vmax=vmax)
else:
	ax.show_colorscale(cmap='gray')
...
#print len(src_ras),len(src_decs)
#print src_ras[:10],src_decs[:10]

#plt.plot(src_ras,src_decs,'o')
#plt.show()
...
ax.show_markers(comp_ras,comp_decs,marker='o',s=30,facecolor='none',edgecolor='#FFFF00',linewidth=2,label="COMPONENT")
ax.show_markers(src_ras,src_decs,marker='o',s=30,facecolor='none',edgecolor='c',linewidth=2,label="BASE SOURCE")
